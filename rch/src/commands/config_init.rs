...
use crate::config::format_default_exclude_patterns_for_toml;
use crate::ui;
...
mod tests {
    ...
    #[test]
    fn generate_config_toml_includes_default_security_excludes() {
        let _guard = test_guard!();
        let values = ConfigValues {
            log_level: "info".to_string(),
            socket_path: "/tmp/rch.sock".to_string(),
        ...
        assert!(toml.contains(r#""*.pem""#));
        assert!(toml.contains(r#""credentials.json""#));
        assert!(toml.contains(r#""target/""#));
        assert!(toml.contains("replaces the built-in defaults"));
    }
